...
class DiarioRepositoryImpl @Inject constructor(
        ...
        callbackFlow {
            val listado = fb.whereEqualTo("idUsuario", idUsuario).addSnapshotListener { sna, _ ->
                ...
                for (i in sna!!.documents) {
                    ...
                    product!!.id = i.id
                    product.time?.let {
                        val time = product.getFecha(it)
                        if (time.diaNum == fecha.diaNum && time.mes == fecha.mes && time.anio == fecha.anio) {
                            lista.add(product)
                        }
                    }
                }
    ...
    override suspend fun delete(idDiario: String?) {
        if (idDiario != null) {
            fb.document(idDiario).delete()
        }
    }
